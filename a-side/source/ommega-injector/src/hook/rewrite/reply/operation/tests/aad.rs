...
#[test]
fn ommega_invalid_update_aad_returns_business_error() {
    ...
    .expect("updateAad rewrite should succeed")
    .expect("ommega invalid updateAad should return an ommega-owned reply");
    let mut reply = reply;
...
#[test]
fn ommega_route_operation_transaction_error_uses_ommega_status_mapping() {
    ensure_binder_process_state();
    let _guard = route_state_test_guard();
    let target = LocalBinderTarget {
        ptr: 0x1234,
    ...
    .expect("ommega transaction status should return an ommega-owned reply");
    // A transport-level failure talking to the ommega operation backend is
