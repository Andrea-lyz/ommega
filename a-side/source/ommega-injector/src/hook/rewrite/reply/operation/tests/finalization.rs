...
#[test]
fn ommega_route_finish_rejects_late_cleanup_abort() {
    ...
    .expect("ommega finish should return an ommega-owned reply");
    let mut reply = reply;
    ...
    assert_eq!(target_info.route, RouteTarget::Ommega);
    ...
    .expect("cleanup abort should return an ommega-owned reply");
...
#[test]
fn ommega_route_abort_clears_operation_mapping() {
    ...
    .expect("ommega abort should return an ommega-owned reply");
    let mut reply = reply;
    let (data, data_size, offsets, offsets_size) = raw_parts(&mut reply);
