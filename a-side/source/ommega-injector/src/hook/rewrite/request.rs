    ...
    if request_interface == identify::KEYSTORE_AUTHORIZATION_INTERFACE {
        ...
        if requires_mirror && !expects_reply {
            if failure_policy == MirrorFailurePolicy::BestEffort {
                warn!(
                    "event=mirror cannot confirm one-way best-effort authorization {:?} for uid={} pid={}; preserving System mutation without an ommega mirror",
        ...
        if requires_mirror {
            match reserve_mirror_update(MirrorStateKind::Authorization, failure_policy) {
                ...
                Err(error) if failure_policy == MirrorFailurePolicy::BestEffort => {
                    warn!(
                        "event=mirror failed to reserve in-memory best-effort authorization {:?} update for uid={} pid={}: {:#}; preserving System mutation without an ommega mirror",
