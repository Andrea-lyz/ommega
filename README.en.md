...
Align patch levels upwards, never downwards. A key blob minted or upgraded under a newer value refuses to be upgraded once the device reports an older one: every later sign and attestation for those keys fails with `INVALID_ARGUMENT` (-38), and nothing on the module side can undo it. Measured on a B device whose reported value had been lowered: 2371 failed signatures in one day, zero after the value was restored. A saved baseline is not an original value either, because it may have been recorded while an override was already active. `spl-control.sh` therefore refuses writes below the effective value: the boot-time `apply` warns on stderr, an interactive `save` fails and the WebUI shows the reason, and `/data/adb/ommega/allow-spl-downgrade` forces one deliberate downgrade. When two devices must agree, raise the older one towards the newer patch date; keep A's `[trust]` on `auto` and do not lower A to an older B date (measured: Google Wallet card binding fails, and works again once A reports its own values).
